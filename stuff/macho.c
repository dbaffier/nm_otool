...
static int section_check(struct segment_command_64 *sg64, uint64_t big_size, unsigned long size, uint32_t shd)
{
...
static int segment_check(struct load_command *lc, unsigned long size, uint32_t shd)
{
...
static int lc_check(void *file, struct mach_header_64 *mh64, unsigned long size)
{
    uint32_t        i;
    int             err;
    ...
    while (i < mh64->ncmds)
    {
        ...
        if (big_load_end > size)
            return (ERR_CMDSIZE);
        if (lc->cmdsize == 0)
            return (ERR_CMDSIZE);
        if (lc->cmd == LC_SEGMENT_64)
            if ((err = segment_check(lc, size, sizeofhdr)) > 0)
                return (err);
        i++;
...
static int		Macho_64(uint32_t magic)
{
	return (magic == MH_MAGIC_64 || magic == MH_CIGAM_64);
}

static int      integrity_leave(void *file, unsigned long size, int err)
{
    if (file)
        munmap(file, size);
    return (err);
